...
Sprint 30 — Hardening P0+P1. (T1) Confirm invoice normalizes price to ingredient's unit instead of overwriting `ingredient.unit` — uses `convert_quantity(1, ing_unit, line_unit)` factor. (T2) Confirm button disabled when 0 lines assigned + amber warning message. (T3) Delete buttons visible on mobile (`md:opacity-0 md:group-hover:opacity-100`) in Recipes/Invoices. (T4) 404 catch-all route in App.tsx. (T5) AlertTriangle "Choisis un ingrédient" banner on unassigned lines. (T6) Pre-confirm summary (X lignes, Y créations, Z ignorées). (T7) "Importer une autre facture" button in success screen. (T8) InvoiceReview responsive: grid-cols-1 sm:grid-cols-2 header, flex-wrap line headers. Previous: Sprint 29b (unit sync on confirm), Sprint 29 (unit conversion), Sprint 28b (batch recipe pre-fill), Sprint 28 (autoSuggested reset), Sprint 27 (re-confirm/patch/delete confirmed invoices), Sprint 26 (RecipeLinker chip component), Sprint 25 (ingredient chips). See @PLAN.md for original roadmap.

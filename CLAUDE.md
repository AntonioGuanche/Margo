...
Sprint 38 complete — Fix onboarding confirm crash (calculate_food_cost tuple mismatch depuis refactoring unités sprint 29-34 : tuple 2→4 éléments). Fix proxy Vite pour /admin et /webhooks en dev local. Remplacement datetime.utcnow() déprécié par datetime.now(timezone.utc).replace(tzinfo=None) (compatible TIMESTAMP WITHOUT TIME ZONE PostgreSQL). Invoice delete rendu FK-safe (SET NULL sur price_history.invoice_id avant suppression) + migration Alembic ondelete=SET NULL.

### Sprint history
Previous: Sprint 37 (reset DB depuis Paramètres + retrait fix-inflated-prices), Sprint 36 (fix-inflated-prices + sidebar sticky), Sprint 35 (admin panel + query limit 500), Sprint 34 (normalisation unités — migration avec sur-inflation), Sprint 33 (P4 types partagés), Sprint 32 (P3 fiabilité), Sprint 31 (P2 navigation), Sprint 30 (hardening P0+P1), Sprint 29b (unit sync on confirm), Sprint 29 (unit conversion), Sprint 28b (batch recipe pre-fill), Sprint 28 (autoSuggested reset), Sprint 27 (re-confirm/patch/delete confirmed invoices), Sprint 26 (RecipeLinker chip component), Sprint 25 (ingredient chips). See @PLAN.md for original roadmap.

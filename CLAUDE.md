...
- **Invoice line persistence:** User's ingredient assignments and ignored status are saved in the `extracted_lines` JSONB via PATCH. Frontend auto-saves (debounced 1.5s) on ingredient/ignore changes. GET reads back the user's choices, not just the initial OCR match.

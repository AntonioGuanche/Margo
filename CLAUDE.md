...
Sprint 31 — P2 Navigation & Cohérence. (T1) Search bar + status filter (all/pending/confirmed) in Invoices — backend `search` param on `supplier_name.ilike()`, frontend `useInvoices(status, search)`. (T2) Categories open by default in Recipes.tsx + Ingredients.tsx (`collapsed[category] ?? false`). (T3) Toggle sort category vs food cost in Recipes — flat list sorted by `food_cost_percent` desc. (T4) Limit raised to 500 in useRecipes + useIngredients, amber "X sur Y" warning when truncated. (T5) Supplier name with 📦 + `last_updated` date visible in Ingredients list. (T6) Invoices sorted by `invoice_date DESC NULLS LAST, created_at DESC` instead of just `created_at`. Previous: Sprint 30 (hardening P0+P1), Sprint 29b (unit sync on confirm), Sprint 29 (unit conversion), Sprint 28b (batch recipe pre-fill), Sprint 28 (autoSuggested reset), Sprint 27 (re-confirm/patch/delete confirmed invoices), Sprint 26 (RecipeLinker chip component), Sprint 25 (ingredient chips). See @PLAN.md for original roadmap.

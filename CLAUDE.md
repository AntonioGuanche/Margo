...
Sprint 29b — Fix unit mismatch between invoice prices and ingredient units. Confirm invoice now syncs `ingredient.unit` with invoice line unit (if recognized: g/kg/ml/cl/l/piece/pce). New `POST /api/recipes/recalculate-all` endpoint to batch-recalculate all recipes for a restaurant. Previous: Sprint 29 (unit conversion in food cost calculation), Sprint 28b (batch recipe pre-fill), Sprint 28 (autoSuggested reset), Sprint 27 (re-confirm/patch/delete confirmed invoices), Sprint 26 (RecipeLinker chip component), Sprint 25 (ingredient chips), Sprint 24 (delete confirmed invoices), Sprint 23 (MenuUploadZone, multi-recipe invoice lines). See @PLAN.md for original roadmap.

...
Sprint 39 — Volume lines normalisées en €/l. Purement frontend, 0 backend. `handleConfirm` dans InvoiceReview.tsx auto-convertit les lignes avec `volume_liters` détecté : `unit='l'`, `unit_price = Math.abs(total_price) / (Math.abs(quantity) * volume_liters)`. `cleanIngredientName()` dans InvoiceLineCard.tsx nettoie la description lors de la création d'ingrédient (supprime "50 L", "fût", "bag in box", "ifk", etc.). Indicateur "💡 sera stocké à X.XX €/l" dans la bande bleue volume + badge "— en €/l" dans le chip ingrédient. RecipeLinker : prop optionnelle `volumeInfo{servingCl}` → defaults `qty=servingCl/unit='cl'` pour les nouveaux liens recette sur lignes volume.

### Sprint history
Previous: Sprint 38 (onboarding confirm crash + proxy vite + utcnow + invoice delete FK-safe), Sprint 37 (reset DB depuis Paramètres + retrait fix-inflated-prices), Sprint 36 (fix-inflated-prices + sidebar sticky), Sprint 35 (admin panel + query limit 500), Sprint 34 (normalisation unités — migration avec sur-inflation), Sprint 33 (P4 types partagés), Sprint 32 (P3 fiabilité), Sprint 31 (P2 navigation), Sprint 30 (hardening P0+P1), Sprint 29b (unit sync on confirm), Sprint 29 (unit conversion), Sprint 28b (batch recipe pre-fill), Sprint 28 (autoSuggested reset), Sprint 27 (re-confirm/patch/delete confirmed invoices), Sprint 26 (RecipeLinker chip component), Sprint 25 (ingredient chips). See @PLAN.md for original roadmap.
